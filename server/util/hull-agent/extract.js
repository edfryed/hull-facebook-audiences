...
export default class Extract {
   ...
   * @return Promise
   */
  handle({ url, format }, chunkSize, callback) {
   ...
   * @return {Promise}
   */
  request({ segment = null, format = "json", path = "batch", fields = [], additionalQuery = {} }) {
    const { hostname } = this.req;
    const search = _.merge((this.req.query || {}), additionalQuery);
    if (segment) {
